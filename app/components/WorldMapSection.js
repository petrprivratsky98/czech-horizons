...
const navstiveneZemeQuery = `*[_type == "realizovanyProjekt" && defined(zeme)] { zeme, mesto }`
...
export default async function WorldMapSection() {
  ...
  const agregovane = {}
  projekty.forEach((p) => {
    if (!agregovane[p.zeme]) agregovane[p.zeme] = {zeme: p.zeme, pocet: 0, mesta: []}
    ...
    <section id="mapa" style={{
      ...
      <div style={{maxWidth: 1600, margin: '0 auto'}}>
        <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 'clamp(40px, 5vw, 80px)', flexWrap: 'wrap', gap: 32}}>
          ...
          <div style={{maxWidth: 440}}>
            ...
            <div style={{display: 'flex', gap: 24, flexWrap: 'wrap'}}>
              <div>
                <div style={{fontSize: 'clamp(11px, 0.85vw, 14px)', letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 700, color: C.yellow, marginBottom: 6}}>{t('countriesLabel')}</div>
                <div style={{fontSize: 'clamp(36px, 4vw, 64px)', fontWeight: 800, lineHeight: 1, color: C.orange}}>{pocetZemi}</div>
              </div>
              <div>
                <div style={{fontSize: 'clamp(11px, 0.85vw, 14px)', letterSpacing: '0.14em', textTransform: 'uppercase', fontWeight: 700, color: C.yellow, marginBottom: 6}}>{t('projectsLabel')}</div>
        ...
        <div className="grid-map-section" style={{
          ...
          gap: 'clamp(16px, 2vw, 32px)', alignItems: 'start',
        }}>
          <div style={{
            background: `${C.cream}04`, border: `1.5px solid ${C.cream}15`,
            borderRadius: 28, padding: 'clamp(16px, 2vw, 32px)',
          ...
          <div style={{
            background: `${C.cream}04`, border: `1.5px solid ${C.cream}15`,
            borderRadius: 28, padding: 'clamp(24px, 2.5vw, 36px)',
            ...
            <div style={{
              aspectRatio: '1 / 1', maxWidth: 200, width: '100%', alignSelf: 'center',
              borderRadius: 16, overflow: 'hidden',
              backgroundImage: 'url(/reunion.png)', backgroundSize: 'contain',
            ...
            <p style={{fontSize: 'clamp(14px, 1.1vw, 17px)', lineHeight: 1.6, color: `${C.cream}aa`, margin: 0}}>
              {t('reunionDesc')}
            </p>
            <div style={{
              paddingTop: 16, marginTop: 'auto', borderTop: `1px dashed ${C.cream}20`,
